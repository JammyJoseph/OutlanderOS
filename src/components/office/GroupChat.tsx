  ...
  if (msg.type === 'delegation') {
    ...
    return (
      <div className="flex items-center gap-2 py-0.5">
        ...
        <div className="flex items-center gap-1.5 rounded-full border border-neutral-700 bg-neutral-900 px-3 py-1 text-[11px]">
    ...
    return (
      <div className="flex justify-end">
        <div className="max-w-[75%]">
          <div className="rounded-2xl rounded-br-sm bg-neutral-800 px-4 py-2.5 text-sm text-white leading-relaxed"
            dangerouslySetInnerHTML={{ __html: renderMarkdown(msg.content) }} />
          <p className="mt-1 text-right text-[10px] text-neutral-600">{formatTime(msg.timestamp)}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex items-end gap-2">
      ...
      <div className="max-w-[75%]">
        ...
        <div
          className="rounded-2xl rounded-bl-sm bg-neutral-900 px-4 py-2.5 text-sm text-neutral-100 leading-relaxed"
          style={{ borderLeft: `2px solid ${color}66` }}
          dangerouslySetInnerHTML={{ __html: renderMarkdown(msg.content) }}
        />
        <p className="mt-1 text-[10px] text-neutral-600">{formatTime(msg.timestamp)}</p>
      </div>
    </div>
  )
}
...
export function GroupChat({ selectedAgentId }: GroupChatProps) {
  ...
  async function handleSend() {
    ...
    if (target !== 'operations') {
      ...
      // Reset agent back to idle after a bit
      setTimeout(() => {
        updateAgentStatus(target, 'idle')
        setActiveAgent(null)
      }, 4000)
    } else {
      // Operations handles directly
  ...
  return (
    <div className="flex h-full flex-col bg-zinc-900">
      ...
      <div className="flex items-center justify-between border-b border-neutral-800 px-4 py-3">
        <div>
          <h2 className="text-sm font-semibold text-white">Team Channel</h2>
          <p className="text-[11px] text-neutral-500">All agents · {AGENT_FLEET.length} members</p>
        </div>
      ...
      {selectedAgent && (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-neutral-400 truncate">{selectedAgent.role}</p>
            {selectedAgent.currentTask && (
              <p className="text-[10px] text-neutral-600 truncate">{selectedAgent.currentTask}</p>
            )}
          </div>
          <span
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        {isTyping && (
          <div className="flex items-end gap-2">
            ...
            <div
              className="rounded-2xl rounded-bl-sm bg-neutral-900 px-4 py-3"
      ...
      <div className="border-t border-neutral-800 p-3">
        <div className="flex items-end gap-2 rounded-xl border border-neutral-700 bg-neutral-800 px-3 py-2.5 focus-within:border-[#D4A853]/40 transition-colors">
          <textarea
            ref={inputRef}
            rows={1}
            value={input}
            ...
            className="flex-1 resize-none bg-transparent text-sm text-white placeholder-neutral-500 outline-none"
            style={{ maxHeight: 100 }}
          />
          <button
            onClick={handleSend}
            disabled={!input.trim()}
            className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[#D4A853] text-neutral-950 transition-opacity disabled:opacity-30 hover:bg-[#C49843]"
          >
            <Send className="h-3.5 w-3.5" />
          </button>
        </div>
        <p className="mt-1 text-center text-[10px] text-neutral-700">
