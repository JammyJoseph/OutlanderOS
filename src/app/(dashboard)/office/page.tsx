...
function renderMarkdown(raw: string): string {
  ...
  // Extract code blocks to protect from line-break substitution
  ...
  codeBlocks.forEach((code, idx) => {
    ...
    text = text.replace(
      `\x00CODE${idx}\x00`,
      `<pre style="background:#1e293b;color:#e2e8f0;border-radius:8px;padding:12px 14px;overflow-x:auto;margin:8px 0;font-size:0.8em;font-family:monospace;white-space:pre"><code>${esc}</code></pre>`
    )
  })
...
export default function AskOSPage() {
  ...
  async function send() {
    ...
    try {
      ...
      setMessages(prev => [
        ...prev,
        { role: 'assistant', content: data.operationsMessage || 'No response', timestamp: new Date().toISOString() },
      ])
    } catch {
      setMessages(prev => [
        ...prev,
        { role: 'assistant', content: 'Connection failed. Check API configuration.', timestamp: new Date().toISOString() },
      ])
    }
    setLoading(false)
            ...
            {topics.length === 0 ? (
              ...
              <ul className="space-y-0.5">
                {topics.map((t, i) => (
                  <li
                    key={i}
                    className="cursor-default truncate rounded px-2 py-1.5 text-xs text-gray-600 hover:bg-gray-100"
                    title={t}
                  >
                    {t}
                  </li>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex items-start gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
                ...
                {msg.role === 'user' ? (
                  <div className="max-w-[60%] rounded-2xl bg-[#D4A853] px-4 py-3 text-sm text-white">
                    {msg.content}
                  </div>
                ) : (
                  <div
                    className="max-w-[75%] rounded-2xl border border-gray-100 bg-gray-50 px-4 py-3 text-sm text-gray-800 leading-relaxed"
          ...
          <div className="border-t border-gray-100 bg-white px-6 py-4">
            <div className="flex items-center gap-3 rounded-2xl border border-gray-200 bg-gray-50 px-4 py-2 focus-within:border-[#D4A853] transition-colors">
              ...
              <button
                ...
                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-[#D4A853] text-white disabled:opacity-40 hover:bg-[#C49843] transition-colors"
              >
                <Send size={14} />
              </button>
            </div>
