...
import { Check, ChevronDown, ChevronRight, Loader2, Mail } from "lucide-react";
...
function startDateLabel(iso: string | null): string | null {
  if (!iso) return null;
  const diff = daysUntil(iso);
  if (diff === null) return null;
  if (diff < 0) return null; // start date passed, don't show
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <span
          className={`h-2 w-2 shrink-0 rounded-full ${PRIORITY_DOT[item.priority] || "bg-gray-300"}`}
          title={item.priority}
        />
      ...
      {isEmail && (item.emailSnippet || item.emailFrom || startLabel) && (
        <div className="ml-8 mt-1 space-y-0.5">
          ...
          {item.emailSnippet && (
            <p className="truncate text-[11px] italic text-gray-400">
              {item.emailSnippet}
            </p>
          )}
          {startLabel && (
            <p className="text-[11px] text-gray-300">{startLabel}</p>
          )}
        </div>
      )}
    </li>
...
export function UnifiedTaskList({
  ...
  return (
    <div>
      {open.length === 0 ? (
        <p className="py-6 text-center text-sm text-gray-400">Nothing open here. Nice.</p>
      ) : (
        <ul className="space-y-1.5">
          {open.map((it) => (
            <Row key={it.id} item={it} busy={busyId === it.id} onToggle={() => onToggle(it)} />
          ))}
        </ul>
      )}
