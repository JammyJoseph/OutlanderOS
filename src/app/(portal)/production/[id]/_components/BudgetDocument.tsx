...
export function BudgetDocument({ data }: { data: BudgetDocumentData }) {
  ...
  // Drop lines nobody filled in. "Fill template" seeds a role per standard crew
  // position at £0, and printing forty blank rows buries the real budget. A line
  // survives if it carries money, spend, or a description someone typed — so a
  // deliberate £0 line stays, and untouched template rows go.
  const used = (l: BudgetLineItem) =>
    (l.budgeted || 0) !== 0 ||
    (l.actual || 0) !== 0 ||
    (l.invoicedAmount || 0) !== 0 ||
    Boolean(l.description?.trim());
  const groupedUsed: Record<string, BudgetLineItem[]> = Object.fromEntries(
    Object.entries(grouped).map(([k, lines]) => [k, (lines ?? []).filter(used)])
  );

  // Only sections that still carry lines — an empty Post-Production heading in a
  // printed budget is just noise.
  const populated = sections.filter((s) => (groupedUsed[s.key] ?? []).length > 0);
  const hiddenCount =
    Object.values(grouped).reduce((n, l) => n + (l?.length ?? 0), 0) -
    Object.values(groupedUsed).reduce((n, l) => n + l.length, 0);
  ...
  return (
    <div style={docStyle}>
      ...
      {populated.length === 0 ? (
        ...
        populated.map((sec) => {
          const lines = groupedUsed[sec.key] ?? [];
      ...
      >
        <span>
          ...
          {hiddenCount > 0 && (
            <>
              {" "}
              {hiddenCount} unused line{hiddenCount === 1 ? "" : "s"} omitted.
            </>
          )}
        </span>
