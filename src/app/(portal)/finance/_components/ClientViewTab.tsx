...
function StatTile({ label, value, cls }: { label: string; value: string; cls?: string }) {
  return (
    <div className="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900 px-4 py-3 shadow-sm">
      <p className="mb-1 text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">{label}</p>
      <p className={`font-mono text-lg font-bold tabular-nums ${cls ?? 'text-gray-900 dark:text-gray-100'}`}>{value}</p>
    </div>
  )
}
      ...
      <div className="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-gray-500 dark:text-gray-400">
        {seg.map((s) => (
          <span key={s.label} className="inline-flex items-center gap-1">
            <span className={`h-2 w-2 rounded-full ${s.color}`} /> {s.label} <span className="font-mono text-gray-700 dark:text-gray-300">{fmtGBP(s.value)}</span>
          </span>
        ))}
      </div>
    </div>
...
function ClientCard({ r, onOpen }: { r: ClientRollup; onOpen: () => void }) {
  return (
    <div
      ...
      className="cursor-pointer rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900 p-4 shadow-sm transition-shadow hover:shadow-md"
    >
      <div className="mb-3 flex items-center justify-between gap-2">
        <p className="truncate text-sm font-semibold text-gray-900 dark:text-gray-100">{r.clientName}</p>
        <span className="shrink-0 rounded-full bg-gray-100 dark:bg-gray-800 px-2 py-0.5 text-[10px] font-semibold text-gray-500 dark:text-gray-400">
          {r.dealCount} project{r.dealCount === 1 ? '' : 's'}
        </span>
      </div>
      <dl className="mb-3 grid grid-cols-2 gap-x-3 gap-y-1.5 text-[11px]">
        <div>
          <dt className="text-gray-400 dark:text-gray-500">Budget (exc. VAT)</dt>
          <dd className="font-mono font-semibold text-gray-900 dark:text-gray-100">{fmtGBP(r.totalBudget)}</dd>
        </div>
        <div>
          <dt className="text-gray-400 dark:text-gray-500">Spent</dt>
          <dd className={`font-mono font-semibold ${r.totalSpent > r.totalBudget ? 'text-red-500 dark:text-red-400' : 'text-gray-900 dark:text-gray-100'}`}>{fmtGBP(r.totalSpent)}</dd>
        </div>
        <div>
          <dt className="text-gray-400 dark:text-gray-500">Outstanding</dt>
          <dd className={`font-mono ${r.totalOutstanding < 0 ? 'text-red-500 dark:text-red-400' : 'text-gray-700 dark:text-gray-300'}`}>{fmtGBP(r.totalOutstanding)}</dd>
        </div>
        <div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100">{r.clientName}</h2>
          <p className="text-xs text-gray-500 dark:text-gray-400">{r.dealCount} project{r.dealCount === 1 ? '' : 's'} · last active {fmtDate(r.updatedAt)}</p>
        </div>
        {r.clientId && (
          <Link
      ...
      <div className="rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900 p-5 shadow-sm">
        <p className="mb-3 text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">Spending Breakdown</p>
        <BreakdownBar media={r.mediaSpend} production={r.productionCost} margin={r.margin} />
      </div>

      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900 shadow-sm">
        <div className="border-b border-gray-100 dark:border-gray-800 bg-gray-50/70 dark:bg-gray-800/70 px-4 py-2.5">
          <p className="text-xs font-bold uppercase tracking-widest text-gray-700 dark:text-gray-300">Projects</p>
        </div>
        ...
        <ul className="divide-y divide-gray-50 dark:divide-gray-800">
          {r.projects.map((p) => (
            ...
            >
              ...
              <div className="text-right font-mono text-gray-900 dark:text-gray-100 sm:col-span-2">
                <span className="text-gray-400 dark:text-gray-500 sm:hidden">Budget </span>{fmtGBP(p.budgetExVat)}
              </div>
              <div className={`text-right font-mono sm:col-span-2 ${p.spent > p.budgetExVat ? 'text-red-500 dark:text-red-400' : 'text-gray-700 dark:text-gray-300'}`}>
                <span className="text-gray-400 dark:text-gray-500 sm:hidden">Spent </span>{fmtGBP(p.spent)}
              </div>
              <div className="sm:col-span-2">
                <StatusBadge status={displayStatus(p.status)} rawKey={p.status} map={BUDGET_STATUS_STYLES} />
              </div>
              <div className="text-right font-mono text-[10px] text-gray-400 dark:text-gray-500 sm:col-span-2">{fmtDate(p.updatedAt)}</div>
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {archivedCount > 0 && (
          <button
            ...
            className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${showArchived ? 'bg-gray-300 dark:bg-gray-600 text-gray-800 dark:text-gray-200' : 'border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}`}
          >
